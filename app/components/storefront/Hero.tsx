...
<circle cx="462.975" cy="4.04169" r="2.32144" transform="rotate(-18.2968 462.975 4.04169)" fill="white"/>
      ...
      {/* Image Carousel */}
      <Carousel className="relative z-20 mt-5">
        <CarouselContent>
          {data.map((item: Banner) => (
            <CarouselItem key={item.id} className="flex flex-col items-center">
              {/* Image Display */}
              <div className="relative w-[220px] h-[250px] lg:w-[270px] lg:h-[250px] flex items-end">
  ...
  <div className="relative w-full h-full flex flex-col items-center">
    ...
    <div className="absolute bottom-0 w-100% bg-white rounded-2xl shadow-lg px-4 py-3 flex flex-col items-center">
      <h1 className="text-md lg:text-lg font-semibold text-black">{item.title}</h1>
      
      <div className="flex items-center gap-2">
        <p className="text-lg font-bold text-black">₹{item.pricing}</p>
        {item.pricing && (
...
/* <CarouselPrevious className="ml-16 z-30" />
<CarouselNext className="mr-16 z-30" /> */
  );
}
